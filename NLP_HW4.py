...
def read_novel(path_in, path_out):  # 读取语料内容
    ...
    for name in names:
        ...
        for line in open(novel_name, 'r', encoding='gb2312'):
            line.strip('\n')
...
if __name__ == '__main__':   ##
    [data_txt, files] = read_novel("C:/Users/user/Desktop/NLP4/HW4/data", "C:/Users/user/Desktop/NLP4/HW4/output")
    # model = Word2Vec(data_txt, vector_size=400, window=5, min_count=5, epochs=200, workers=multiprocessing.cpu_count())
    test_name = ['张无忌', '乔峰', '郭靖', '杨过', '韦小宝']
    # test_wugong = ['九阳神功', '降龙十八掌', '降龙十八掌', '']
    test_menpai = ['明教', '逍遥派', '少林', '全真教', '少林']
    for i in range(0, 5):
        name = "../深度学习与自然语言处理/output/" + files[i]
        print(name)
